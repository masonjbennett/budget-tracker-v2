...
export default function IncomePage() {
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold text-primary">Income Setup</h1>
      <p className="text-dim mt-1 mb-6">Configure your salary, deductions, and tax situation to calculate your true take-home pay.</p>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
        {/* Left column */}
        <div className="space-y-4">
          <h2 className="text-lg font-bold text-primary">Salary & Location</h2>
          <div>
            <label className="block text-sm font-medium text-dim mb-1">Annual Gross Salary ($)</label>
            <input type="number" value={income.gross_salary} onChange={(e) => update("gross_salary", +e.target.value)}
              className="w-full border border-border rounded-lg px-4 py-2.5 text-primary focus:border-accent focus:ring-2 focus:ring-accent/20 outline-none transition-all" />
          </div>
          <div>
            ...
            <select value={income.state} onChange={(e) => update("state", e.target.value)}
              className="w-full border border-border rounded-lg px-4 py-2.5 text-primary focus:border-accent focus:ring-2 focus:ring-accent/20 outline-none bg-white">
          ...
          <div>
            <label className="block text-sm font-medium text-dim mb-1">Filing Status</label>
          ...
          <div>
            <label className="block text-sm font-medium text-dim mb-1">Bonus Type</label>
          ...
          {income.bonus_type !== "None" && (
            <div>
              <label className="block text-sm font-medium text-dim mb-1">Bonus Amount ($)</label>
              <input type="number" value={income.bonus_amount} onChange={(e) => update("bonus_amount", +e.target.value)}
                className="w-full border border-border rounded-lg px-4 py-2.5 text-primary focus:border-accent focus:ring-2 focus:ring-accent/20 outline-none" />
            </div>
          )}
        </div>
        ...
        {/* Right column */}
        <div className="space-y-4">
          <h2 className="text-lg font-bold text-primary">Pre-Tax Deductions</h2>
          <div>
            <label className="block text-sm font-medium text-dim mb-1">401(k) Contribution (%)</label>
            <input type="range" min={0} max={100} value={income.contribution_401k}
              onChange={(e) => update("contribution_401k", +e.target.value)}
              className="w-full accent-accent" />
            <p className="text-sm text-dim mt-1">{income.contribution_401k}% = {fmt(income.gross_salary * income.contribution_401k / 100)}/year</p>
          </div>
          <div>
            <label className="block text-sm font-medium text-dim mb-1">Health Insurance ($/month)</label>
            <input type="number" value={income.health_insurance} onChange={(e) => update("health_insurance", +e.target.value)}
              className="w-full border border-border rounded-lg px-4 py-2.5 text-primary focus:border-accent focus:ring-2 focus:ring-accent/20 outline-none" />
          </div>
          <div>
            <label className="block text-sm font-medium text-dim mb-1">HSA Contribution ($/month)</label>
            <input type="number" value={income.hsa} onChange={(e) => update("hsa", +e.target.value)}
              className="w-full border border-border rounded-lg px-4 py-2.5 text-primary focus:border-accent focus:ring-2 focus:ring-accent/20 outline-none" />
          </div>
          <div>
            <label className="block text-sm font-medium text-dim mb-1">Student Loan Interest ($/year)</label>
            <input type="number" value={income.student_loan_interest} max={2500}
              ...
              className="w-full border border-border rounded-lg px-4 py-2.5 text-primary focus:border-accent focus:ring-2 focus:ring-accent/20 outline-none" />
            <p className="text-xs text-dim mt-1">Max $2,500. Above-the-line deduction.</p>
          </div>
        </div>
      </div>
      ...
      {result && (
        <div className="mt-8">
          ...
          <p className="text-dim text-sm mb-4">Your estimated net pay after all taxes and deductions.</p>
