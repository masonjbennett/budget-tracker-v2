...
export default function TaxPage() {
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold text-primary">Tax Estimator</h1>
      <p className="text-dim mt-1 mb-2">Estimate your federal and state tax liability, compare deduction strategies, and optimize contributions.</p>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div>
          <label className="block text-sm font-medium text-dim mb-1">Gross Salary ($)</label>
        ...
        <div>
          ...
          <select value={income.state} onChange={(e) => setIncome({...income, state: e.target.value})}
            className="w-full border border-border rounded-lg px-4 py-2.5 focus:border-accent focus:ring-2 focus:ring-accent/20 outline-none bg-white">
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-6 mb-8">
            <div className="bg-white border border-border rounded-xl p-5">
              <h3 className="font-bold text-primary mb-2">Tax Rates</h3>
              <div className="flex justify-between">
                <div>
                  <p className="text-xs text-dim uppercase tracking-wider">Effective Rate</p>
                  <p className="text-2xl font-bold text-yellow">{result.effective_rate.toFixed(1)}%</p>
                  <p className="text-xs text-dim">What you actually pay overall</p>
                </div>
                <div>
                  <p className="text-xs text-dim uppercase tracking-wider">Marginal Rate</p>
                  <p className="text-2xl font-bold text-red">{result.marginal_fed.toFixed(0)}%</p>
                  <p className="text-xs text-dim">Tax on your next dollar</p>
                </div>
              </div>
            </div>
            <div className="bg-white border border-border rounded-xl p-5">
              <h3 className="font-bold text-primary mb-2">Deduction Breakdown</h3>
              <p className="text-sm text-dim">Filing: <strong>{result.filing}</strong></p>
              <p className="text-sm text-dim">AGI: <strong>{fmt(result.agi)}</strong></p>
              <p className="text-sm text-dim">Standard Deduction: <strong>-{fmt(result.std_ded)}</strong></p>
              <p className="text-sm text-dim">Taxable Income: <strong>{fmt(result.taxable)}</strong></p>
            </div>
          </div>
        </>
      )}
      ...
      {rothResult && (
        <>
          <hr className="border-border mb-8" />
          <h2 className="text-xl font-bold text-primary mb-1">Roth vs. Traditional 401(k)</h2>
          <p className="text-dim text-sm mb-4">Compare after-tax ending values based on your current vs expected retirement tax rate.</p>
          <div className="mb-4">
            <label className="block text-sm font-medium text-dim mb-1">Expected Retirement Tax Rate (%)</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className={`bg-white border rounded-xl p-5 ${rothResult.better === "Traditional" ? "border-green border-l-4" : "border-border"}`}>
              <h3 className="font-bold text-primary">Traditional (Pre-Tax)</h3>
              <p className="text-2xl font-bold mt-1">{fmt(rothResult.traditional_after_tax)}</p>
              <p className="text-xs text-dim mt-1">After {futureRate}% tax on withdrawal</p>
              {rothResult.better === "Traditional" && <p className="text-sm text-green font-semibold mt-2">Better by {fmt(rothResult.difference)}</p>}
            </div>
            <div className={`bg-white border rounded-xl p-5 ${rothResult.better === "Roth" ? "border-green border-l-4" : "border-border"}`}>
              <h3 className="font-bold text-primary">Roth (Post-Tax)</h3>
              <p className="text-2xl font-bold mt-1">{fmt(rothResult.roth_future)}</p>
              <p className="text-xs text-dim mt-1">Tax-free withdrawal</p>
