...
export default function Dashboard() {
  ...
  // Fake sparkline data (would come from historical snapshots)
  const spendSpark = [2100, 2300, 1900, 2600, 2400, 2200, totalSpent];
  const nwSpark = [21000, 21800, 22500, 23200, 23800, 24200, netWorth];

  return (
    <div>
      <PageHeader title="Financial Health Dashboard" description="Your financial overview — updates live as you adjust income, log expenses, and track net worth." />

      {/* Cmd+K hint */}
      <div className="mb-6 flex items-center gap-2 text-[0.72rem] text-slate-600">
        <kbd className="bg-[#1e293b] px-1.5 py-0.5 rounded border border-[#334155] text-slate-500">⌘K</kbd>
        <span>to search and navigate</span>
      </div>

      {/* Hero metrics */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 stagger">
        <MetricCard label="Monthly Take-Home" value={fmt(monthly)} help="Net pay after taxes and deductions" gradient />
        <MetricCard label="Spent This Month" value={fmt(totalSpent)} delta={`${monthExpenses.length} transactions`} sparkData={spendSpark} />
        <MetricCard label="Net Worth" value={fmt(netWorth)} sparkData={nwSpark} />
      ...
      <div className="mt-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 stagger">
          <StatusCard label="Savings Rate" value={`${savingsRate.toFixed(1)}%`}
            ...
            description="20%+ is excellent. 10-20% is solid." />
          <StatusCard label="Debt-to-Income" value={`${dti.toFixed(1)}%`}
            ...
            description="Below 20% is great. Above 36% limits borrowing." />
      ...
      {catLabels.length > 0 && (
        <div className="mt-10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <Chart
              data={[{
                labels: catLabels, values: catValues, type: "pie", hole: 0.5,
      ...
      <div className="mt-10">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div className="flex-1 h-px bg-white/[0.06]" />
          <span className="text-[0.7rem] text-slate-600">Synced with Income page</span>
        </div>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 stagger">
          ...
          <MetricCard label="Take-Home Pay" value={fmt(takeHome.annual_take_home)} delta={`${takeHome.effective_rate.toFixed(1)}% effective`} deltaColor="green" />
        </div>
      </div>
